...
public class OLabelUI extends BasicLabelUI implements SynthUI {
    ...
    protected void installDefaults(JLabel c) {
        ...
        if ("ELabel".equals(compName)) {
            OntimizeLookAndFeel.installColorsAndFont(c, "\"ELabel\".background", "\"ELabel\".foreground",
                    "\"ELabel\".font");
            LookAndFeel.installProperty(c, "opaque", Boolean.FALSE);
        } else if (c instanceof BasicTreeCellRenderer) {
            OntimizeLookAndFeel.installColorsAndFont(c, "Tree:\"Tree.cellRenderer\".background",
                    "Tree:\"Tree.cellRenderer\".foreground", "Tree:\"Tree.cellRenderer\".font");
            LookAndFeel.installProperty(c, "opaque", Boolean.FALSE);
        } else if ("PageFetcher.Label".equals(compName)) {
            OntimizeLookAndFeel.installColorsAndFont(c, "\"PageFetcher.Label\".background",
                    "\"PageFetcher.Label\".foreground", "\"PageFetcher.Label\".font");
            LookAndFeel.installProperty(c, "opaque", Boolean.FALSE);
        } else if ("TableHeader.renderer".equals(compName)) {
            OntimizeLookAndFeel.installColorsAndFont(c, "TableHeader:\"TableHeader.renderer\".background",
                    "TableHeader:\"TableHeader.renderer\".foreground",
                    "TableHeader:\"TableHeader.renderer\".font");
            LookAndFeel.installProperty(c, "opaque", Boolean.FALSE);
        } else if ((compName != null) && (compName.length() > 0)) {
            OntimizeLookAndFeel.installColorsAndFont(c, compName + ".background", compName + ".foreground",
                    compName + ".font");
            LookAndFeel.installProperty(c, "opaque", Boolean.FALSE);
        } else {
    ...
    protected int getComponentState(JComponent c) {
        ...
        /*
         * Making it compatible with older versions of Ontimize that has not got TabbedFormManager
         */
        try {
            if (c.getParent() instanceof com.ontimize.gui.manager.TabbedFormManager.ButtonTabComponent) {
                ...
                if ((com.ontimize.gui.manager.TabbedFormManager.ButtonTabComponent) c.getParent() == tabbedPane
    ...
    public int getBaseline(JComponent c, int width, int height) {
        ...
        context.getStyle()
            ...
            .layoutText(context, fm, label.getText(), label.getIcon(), label.getHorizontalAlignment(),
    ...
    /**
     * Notifies this UI delegate that it's time to paint the specified component. This method is invoked
     * by <code>JComponent</code> when the specified component is being painted.
     */
    @Override
    ...
    protected void paint(SynthContext context, Graphics g) {
        ...
        if (label instanceof VisualCalendarComponent.HeaderRenderer) {
            ...
            Object oColor = OntimizeLookAndFeel
                .lookup("TableHeader:\"VisualCalendar:TableHeader.renderer\".foregroundShadow");
            ...
            context.getStyle()
                .getGraphicsUtils(context)
                .paintText(context, g, label.getText(), null, label.getHorizontalAlignment(),
                        label.getVerticalAlignment(), label.getHorizontalTextPosition(),
        ...
        context.getStyle()
            .getGraphicsUtils(context)
            .paintText(context, g, label.getText(), icon, label.getHorizontalAlignment(), label.getVerticalAlignment(),
    ...
    protected int drawBasicTreeCellRendererChildCount(Graphics2D g, BasicTreeCellRenderer renderer) {
        ...
        Shape circle = ShapeFactory.getInstace()
            .createSemiCircle(height / 2, 0, height, ShapeFactory.ANTICLOCKWISE, false);
        g.fill(circle);
        ...
        circle = ShapeFactory.getInstace()
            .createSemiCircle((height / 2) + textWidth, 0, height, ShapeFactory.CLOCKWISE, false);
        g.fill(circle);

        if (renderer.isSelected()) {
            Object obj = UIManager.getLookAndFeelDefaults()
                .get("Tree:\"Tree.cellRenderer\".foregroundSelectionChildCount");
            if (obj instanceof Paint) {
                g.setPaint((Paint) obj);
            } else {
                g.setColor(renderer.getBackgroundSelectionColor());
    ...
    public Dimension getPreferredSize(JComponent c) {
        ...
        Dimension size = context
            .getStyle()
            .getGraphicsUtils(context)
            .getPreferredSize(context, context.getStyle().getFont(context), label.getText(), icon,
                    label.getHorizontalAlignment(), label.getVerticalAlignment(),
                    label.getHorizontalTextPosition(), label.getVerticalTextPosition(), label.getIconTextGap(),
    ...
    public Dimension getMinimumSize(JComponent c) {
        ...
        Dimension size = context
            .getStyle()
            .getGraphicsUtils(context)
            .getMinimumSize(context, context.getStyle().getFont(context), label.getText(), icon,
                    label.getHorizontalAlignment(), label.getVerticalAlignment(),
                    label.getHorizontalTextPosition(), label.getVerticalTextPosition(), label.getIconTextGap(),
                    label.getDisplayedMnemonicIndex());
    ...
    public Dimension getMaximumSize(JComponent c) {
        ...
        Dimension size = context
            .getStyle()
            .getGraphicsUtils(context)
            .getMaximumSize(context, context.getStyle().getFont(context), label.getText(), icon,
                    label.getHorizontalAlignment(), label.getVerticalAlignment(),
                    label.getHorizontalTextPosition(), label.getVerticalTextPosition(), label.getIconTextGap(),
                    label.getDisplayedMnemonicIndex());
