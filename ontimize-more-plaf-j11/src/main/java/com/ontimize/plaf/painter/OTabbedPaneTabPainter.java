...
/**
 * Painter for the background of each tab in a TabbedPane (to configure border and background
...
public class OTabbedPaneTabPainter extends AbstractRegionPainter {
    ...
    float[] nineFractions = new float[] { 0.0f, 0.12419355f, 0.2483871f, 0.42580646f, 0.6032258f, 0.6854839f,
    ...
    protected void init() {
        ...
        obj = UIManager.getLookAndFeelDefaults()
            .get(this.getComponentKeyName() + "[Focused+MouseOver+Selected].background");
        if (obj instanceof Paint) {
        ...
        obj = UIManager.getLookAndFeelDefaults()
            .get(this.getComponentKeyName() + "[Focused+Pressed+Selected].background");
        if (obj instanceof Paint) {
     ...
     * @param position the position of the tab (first, middle, last or none(none when there is only one
     *        tab) ).
    ...
    protected Shape decodeLastInteriorTabUpperShape(double x, double y, double w) {
        ...
        this.path.curveTo((x + w) - 1, this.decodeY(1.32f), (x + w) - 2.5, this.decodeY(1.4225f), (x + w) - 5,
                this.decodeY(1.4225f)); // y+6;
                                        // y+8
    ...
    protected Shape decodeButtonInteriorTabUpperShape(double x, double y, double w) {
        ...
        this.path.curveTo((x + w) - 2.5, this.decodeY(1.4225f), (x + w) - 1, this.decodeY(1.32f), (x + w) - 1,
                this.decodeY(1.215f)); // y+8
                                       // y+6;
                                       // y+4
        this.path.lineTo((x + w) - 1, y);
    ...
    protected Shape decodeLastInteriorTabBottomShape(double x, double y, double w) {
        ...
        this.path.curveTo(x + w, this.decodeY(3.0f) - 1 - 1.5, (x + w) - 1.5, this.decodeY(3.0f) - 1, (x + w) - 3,
                this.decodeY(3.0f) - 1);
    ...
    protected Shape decodeButtonInteriorTabBottomShape(double x, double y, double w) {
        ...
        this.path.curveTo(x + w, this.decodeY(2.0f) - 1.5, (x + w) - 1.5, this.decodeY(2.0f), (x + w) - 3,
                this.decodeY(2.0f));
    ...
    protected Paint decodeGradientBorderShape(Shape s, boolean focused) {
        ...
        if (focused) {
            return this.decodeGradient(x, y, x, y + h, new float[] { 0.0f, 0.19f, 0.76f, 0.9f, 1.0f },
