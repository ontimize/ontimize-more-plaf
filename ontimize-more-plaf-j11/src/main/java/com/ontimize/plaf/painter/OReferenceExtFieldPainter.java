 ...
 *
 * This kind of extensions painters were developed to allow users to have several componentes which
 * are painted in the same way, and configured by the same set of properties, but having only a
 * common code to paint (the class that this one extends).
 *
 * @author Imatia S.L.
 *
 */
public class OReferenceExtFieldPainter extends AbstractOTextFieldPainter {
    ...
    protected void paintBorderDisabled(Graphics2D g, JComponent c, int x, int y, int width, int height) {
        ...
        if (c.getParent() instanceof ReferenceExtDataField) {
            ...
            if ((ref.isCodeFieldVisible() == false) && (ref.isCodeSearchVisible() == false)
                    && (ref.isEnabled() == false)) {
