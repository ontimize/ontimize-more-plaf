...
public class OTabbedPanePainter extends AbstractRegionPainter {
        ...
        if ((tabPane.getTabCount() > 0) && (tabPane.getUI() instanceof OTabbedPaneUI)) {
            ...
            for (int i = 0; i < OTabbedPanePainter.borderWidth; i++) {
                ...
                g.setPaint(OntimizeLAFColorUtils.setAlpha((Color) color, OTabbedPanePainter.maxAlphaBorderColor
                        - (((OTabbedPanePainter.maxAlphaBorderColor - OTabbedPanePainter.minAlphaBorderColor)
                                / OTabbedPanePainter.borderWidth) * indexColor)));
                g.draw(s);
                indexColor++;
            }
    ...
    protected void paintInteriorBorderFillers(Graphics2D g, double x, double y, double w, double h, int placement,
        ...
        Shape leftFiller = ShapeFactory.getInstace()
            .createSemiRoundRect(x + OTabbedPanePainter.borderWidth, y + OTabbedPanePainter.borderWidth,
                    OTabbedPanePainter.borderPadding,
                    h - totalBorder, arcRadius, arcRadius, ShapeFactory.LEFT_ORIENTATION);
        g.fill(leftFiller);

        Shape rightFiller = ShapeFactory.getInstace()
            .createSemiRoundRect(((w + x) - OTabbedPanePainter.borderPadding - OTabbedPanePainter.borderWidth) + 1,
        ...
        Shape bottomFiller = ShapeFactory.getInstace()
            .createRectangle(x + totalBorder, ((h + y) - totalBorder) + 1,
    ...
    protected Shape decodeBorderPath(double x, double y, double w, double h, double arcRadius) {
        ...
        Shape s = ShapeFactory.getInstace()
            .createRoundCorner(x, y, arcRadius, arcRadius, ShapeFactory.TOP_LEFT_CORNER, true);
        ...
        s = ShapeFactory.getInstace()
            .createRoundCorner(x + w, y, arcRadius, arcRadius, ShapeFactory.TOP_RIGHT_CORNER, true);
