...
public abstract class AbstractRegionPainter implements Painter<JComponent> {
    ...
    /**
     * Added to be able to get the whole area that must be painted, in order to create Ontimize Painters
     * for the whole area. Sometimes, only a part of the whole area will be painted (depending on the
     * specifications in the classes that extends this one).
     * @param path
     * @return
     */
    ...
    /**
     * PaintContext, which holds a lot of the state needed for cache hinting and x/y value decoding The
     * data contained within the context is typically only computed once and reused over multiple paint
     * calls, whereas the other values (w, h, f, leftWidth, etc) are recomputed for each call to paint.
     *
     * This field is retrieved from subclasses on each paint operation. It is up to the subclass to
     * compute and cache the PaintContext over multiple calls.
     */
    ...
    /*
     * Various metrics used for decoding x/y values based on the canvas size and stretching insets.
     *
     * On each call to paint, we first ask the subclass for the PaintContext. From the context we get
     * the canvas size and stretching insets, and whether the algorithm should be "inverted", meaning
     * the center section remains a fixed size and the other sections scale.
     *
     * We then use these values to compute a series of metrics (listed below) which are used to decode
     * points in a specific axis (x or y).
     *
     * The leftWidth represents the distance from the left edge of the region to the first stretching
     * inset, after accounting for any scaling factor (such as DPI scaling). The centerWidth is the
     * distance between the leftWidth and the rightWidth. The rightWidth is the distance from the right
     * edge, to the right inset (after scaling has been applied).
     *
     * The same logic goes for topHeight, centerHeight, and bottomHeight.
     *
     * The leftScale represents the proportion of the width taken by the left section. The same logic is
     * applied to the other scales.
     *
     * The various widths/heights are used to decode control points. The various scales are used to
     * decode bezier handles (or anchors).
     */
    /**
    ...
    /**
     * The scaling factor to use for the center section, in the horizontal direction. Recomputed on each
     * call to paint.
     */
    protected float centerHScale;

    /**
     * The scaling factor to use for the center section, in the vertical direction. Recomputed on each
     * call to paint.
     */
    ...
    /**
     * Get any extra attributes which the painter implementation would like to include in the image
     * cache lookups. This is checked for every call of the paint(g, c, w, h) method.
    ...
    /**
     * <p>
     * Gets the PaintContext for this painting operation. This method is called on every paint, and so
     * should be fast and produce no garbage. The PaintContext contains information such as cache hints.
     * It also contains data necessary for decoding points at runtime, such as the stretching insets,
     * the canvas size at which the encoded points were defined, and whether the stretching insets are
     * inverted.
     * </p>
     *
     * <p>
     * This method allows for subclasses to package the painting of different states with possibly
     * different canvas sizes, etc, into one AbstractRegionPainter implementation.
     * </p>
    ...
    /**
     * <p>
     * Configures the given Graphics2D. Often, rendering hints or compositiing rules are applied to a
     * Graphics2D object prior to painting, which should affect all of the subsequent painting
     * operations. This method provides a convenient hook for configuring the Graphics object prior to
     * rendering, regardless of whether the render operation is performed to an intermediate buffer or
     ...
     * object passed may represent the actual surface being rendererd to, or it may be an intermediate
     * buffer. It has also been pre-translated. Simply render the component as if it were located at 0,
     * 0 and had a width of <code>width</code> and a height of <code>height</code> . For performance
     * reasons, you may want to read the clip from the Graphics2D object and only render within that
     * space.
     ...
     *        is Button, then <code>c</code> will be a JButton. If the region being drawn is
     *        ScrollBarSlider, then the component will be JScrollBar. This value may be null.
     * @param width The width of the region to paint. Note that in the case of painting the foreground,
     *        this value may differ from c.getWidth().
    ...
    /**
     * Decodes and returns a float value representing the actual pixel location for the given encoded X
     * value.
    ...
    /**
     * Decodes and returns a float value representing the actual pixel location for the given encoded y
     * value.
    ...
    /**
     * Decodes and returns a float value representing the actual pixel location for the anchor point
     * given the encoded X value of the control point, and the offset distance to the anchor from that
     * control point.
    ...
    /**
     * Decodes and returns a float value representing the actual pixel location for the anchor point
     * given the encoded Y value of the control point, and the offset distance to the anchor from that
     * control point.
     ...
     * @param key A key corrosponding to the value in the UI Defaults table of UIManager where the base
     *        color is defined
     ...
     * @return The derived color, whos color value will change if the parent uiDefault color changes.
     */
     ...
     *        color 2;
     * @return The derived color, whos color value will change if either of the colors change, this will
     *         only work if they are color fetched from the other decodeColor(s,f,f,f,i) method as they
     *         fire property change events for then they change.
     */
     ...
     */
    protected LinearGradientPaint decodeGradient(float x1, float y1, float x2, float y2, float[] midpoints,
            Color[] colors) {
        if ((x1 == x2) && (y1 == y2)) {
            y2 += .00001f;
        }
    ...
    /**
     * Get a color property from the given JComponent. First checks for a <code>getXXX()</code> method
     * and if that fails checks for a client property with key <code>property</code>. If that still
     * fails to return a Color then <code>defaultColor</code> is returned.
     * @param c The component to get the color property from
     * @param property The name of a bean style property or client property
     * @param defaultColor The color to return if no color was obtained from the component.
     * @return The color that was obtained from the component or defaultColor
     */
    protected Color getComponentColor(JComponent c, String property, Color defaultColor, float saturationOffset,
    ...
    /**
     * A class encapsulating state useful when painting. Generally, instances of this class are created
     * once, and reused for each paint request without modification. This class contains values useful
     * when hinting the cache engine, and when decoding control points and bezier curve anchors.
     */
    public static class PaintContext {
         ...
         *        If null, then it is assumed that there are no encoded values, and any calls to one of the
         ...
         * @param maxH The maximium scale in the horizontal direction to use before punting and redrawing
         *        from scratch. For example, if maxH is 2, then we will attempt to scale any cached images
         *        up to 2x the canvas width before redrawing from scratch. Reasonable maxH values may
         *        improve painting performance. If set too high, then you may get poor looking graphics at
         *        higher zoom levels. Must be >= 1.
         * @param maxV The maximium scale in the vertical direction to use before punting and redrawing from
         *        scratch. For example, if maxV is 2, then we will attempt to scale any cached images up to
         *        2x the canvas height before redrawing from scratch. Reasonable maxV values may improve
         *        painting performance. If set too high, then you may get poor looking graphics at higher
         *        zoom levels. Must be >= 1.
         */
    ...
    protected void prepare(float w, float h) {
        ...
        this.rightScale = (this.ctx.canvasSize.width - this.ctx.b) == 0f ? 0f
                : this.rightWidth / (this.ctx.canvasSize.width - this.ctx.b);
        ...
        this.bottomScale = (this.ctx.canvasSize.height - this.ctx.d) == 0f ? 0f
                : this.bottomHeight / (this.ctx.canvasSize.height - this.ctx.d);
    }

    protected void paintWith9SquareCaching(Graphics2D g, PaintContext ctx, JComponent c, int w, int h,
        ...
        if ((w <= (canvas.width * ctx.maxHorizontalScaleFactor))
            ...
            VolatileImage img = this.getImage(g.getDeviceConfiguration(), c, canvas.width, canvas.height,
                    extendedCacheKeys);
            if (img != null) {
    ...
    /**
     * Gets the rendered image for this painter at the requested size, either from cache or create a new
     * one
     */
    protected VolatileImage getImage(GraphicsConfiguration config, JComponent c, int w, int h,
            Object[] extendedCacheKeys) {
