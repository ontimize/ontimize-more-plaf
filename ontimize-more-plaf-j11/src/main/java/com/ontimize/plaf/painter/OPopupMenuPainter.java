...
/**
 * Painter for the background of the pop up (to configure border and foreground
...
public class OPopupMenuPainter extends AbstractRegionPainter {
	...
	/**
	 * Note that menu and menu item elements are overpainted in the pop up, so
	 * the popup menu background painter can be hidden by menus painters
	 *
	 * @param g
	 */
		...
		if (c instanceof JPopupMenu) {
			...
			if (invoker instanceof JComponent) {
				...
				if (invoker.getParent() instanceof JPopupMenu) {
					...
					s = ShapeFactory.getInstace().createRectangle(this.decodeX(1.0f), this.decodeY(1.0f), this.decodeX(2.0f) - this.decodeX(1.0f),
	...
	/**
	 * Method to know whether child PopupMenu is opened on the right or on the
	 * left of the invoker component. The invoker component is an JMenu placed
	 * into a JPopupMenu.
	 * 
	 * @param invoker
	 *            Component that provokes that a new PopupMenu will be
	 *            displayed.
	 * @param component
	 *            the PopupMenu.
	 * @return true if child PopupMenu is placed on the right of invoker
	 *         component.
	 */
	...
	/**
	 * Method to know whether invoker component is going to open the child
	 * PopupMenu on the right or on the left. The invoker component is an JMenu
	 * placed into a JMenuBar.
	 * 
	 * @param invoker
	 *            Component that provokes that a new PopupMenu will be
	 *            displayed.
	 * @param component
	 *            the PopupMenu.
	 * @return true if invoker component is placed on the left of child
	 *         PopupMenu.
	 */
	...
	protected Shape decodeShadowMenuBar(int x, int y, JComponent invoker, JComponent c) {
		...
		if (leftPos) {
			if (width < compWidth) {
				...
				this.path.lineTo(this.decodeX(3.0f) - 2 - numBorders - arcRadius, (this.decodeY(0.0f) + yOffset + y) - numBorders);
				this.path.curveTo(this.decodeX(3.0f) - x - 1 - (arcRadius / 2.0), (this.decodeY(0.0f) + yOffset + y) - numBorders, this.decodeX(3.0f) - x - 1, ((this.decodeY(0.0f)
						+ yOffset + numBorders) - 1)
						+ (arcRadius / 2.0), this.decodeX(3.0f) - x - 1, ((this.decodeY(0.0f) + yOffset + numBorders) - 1) + arcRadius);
			} else {
			...
			// Bottom right corner...
			this.path.curveTo(this.decodeX(3.0f) - numBorders - x, this.decodeY(3.0f) - y - 1 - (arcRadius / 2.0), this.decodeX(3.0f) - 2 - numBorders - (arcRadius / 2.0),
					this.decodeY(3.0f) - y - 1, this.decodeX(3.0f) - 2 - numBorders - arcRadius, this.decodeY(3.0f) - y - 1);

			double xPos = this.decodeX(0.0f) + xOffset + numBorders + 1;
			this.path.lineTo(xPos + arcRadius, this.decodeY(3.0f) - y - 1);
			// Bottom left corner...
			this.path.curveTo(((this.decodeX(0.0f) + xOffset) - 1) + x + (arcRadius / 2.0), this.decodeY(3.0f) - numBorders - y, this.decodeX(0.0f) + xOffset + x,
					this.decodeY(3.0f) - 3 - numBorders - (arcRadius / 2.0), this.decodeX(0.0f) + xOffset + x, this.decodeY(3.0f) - 3 - numBorders - arcRadius);
			...
			if (width < compWidth) {
				this.path.lineTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset + numBorders) - 1) + arcRadius);
				this.path.curveTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset + y) - numBorders) + (arcRadius / 2.0), this.decodeX(0.0f) + numBorders
						+ xOffset + (arcRadius / 2.0), (this.decodeY(0.0f) + yOffset + y) - numBorders, this.decodeX(0.0f) + numBorders + xOffset + arcRadius, (this.decodeY(0.0f)
						+ yOffset + y)
						- numBorders);
			} else {
	...
	protected Shape decodeShadowPopupMenu(int x, int y, boolean rightPosition) {
		...
		if (rightPosition) {
			...
			this.path.moveTo(this.decodeX(0.0f) + xOffset + x + arcRadius, (this.decodeY(0.0f) + yOffset + y) - numBorders);
			...
			this.path.lineTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset + numBorders) - 1) + arcRadius);
			this.path.curveTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset) - numBorders) + (arcRadius / 2.0), this.decodeX(0.0f) + xOffset + x
					+ (arcRadius / 2.0), (this.decodeY(0.0f) + yOffset + y) - numBorders, this.decodeX(0.0f) + xOffset + x + arcRadius, (this.decodeY(0.0f) + yOffset + y)
					- numBorders);
		}
	...
	protected Shape decodeShadow(int x, int y) {
		...
		this.path.reset();
		this.path.moveTo(this.decodeX(0.0f) + xOffset + numBorders + 1 + arcRadius, (this.decodeY(0.0f) + yOffset + y) - numBorders);
		...
		this.path.lineTo(xPos + arcRadius, this.decodeY(3.0f) - y - 1);
		// Bottom left corner...
		this.path.curveTo(((this.decodeX(0.0f) + xOffset) - 1) + x + (arcRadius / 2.0), this.decodeY(3.0f) - numBorders - y, this.decodeX(0.0f) + xOffset + x, this.decodeY(3.0f)
				- 3 - numBorders - (arcRadius / 2.0), this.decodeX(0.0f) + xOffset + x, this.decodeY(3.0f) - 3 - numBorders - arcRadius);

		this.path.lineTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset + numBorders) - 1) + arcRadius);
		// Top left corner...
		this.path.curveTo(this.decodeX(0.0f) + xOffset + x, ((this.decodeY(0.0f) + yOffset + numBorders) - 1) + (arcRadius / 2.0), this.decodeX(0.0f) + xOffset + numBorders + 1
				+ (arcRadius / 2.0), (this.decodeY(0.0f) + yOffset + y) - numBorders, this.decodeX(0.0f) + xOffset + numBorders + 1 + arcRadius, (this.decodeY(0.0f) + yOffset + y)
				- numBorders);
	...
	protected Shape decodeBorder(JComponent invoker) {
		...
		this.path.curveTo((this.decodeX(2.0f) + numBorders) - (arcRadius / 2.0), this.decodeY(0.0f) + numBorders, this.decodeX(2.0f) + numBorders, this.decodeY(0.0f) + numBorders
				+ (arcRadius / 2.0), this.decodeX(2.0f) + numBorders, this.decodeY(0.0f) + numBorders + arcRadius);
		this.path.lineTo(this.decodeX(2.0f) + numBorders, this.decodeY(3.0f) - numBorders - arcRadius);
		this.path.curveTo(this.decodeX(2.0f) + numBorders, this.decodeY(3.0f) - (2 * numBorders) - (arcRadius / 2.0), (this.decodeX(2.0f) + numBorders) - (arcRadius / 2.0),
				this.decodeY(3.0f) - (2 * numBorders), (this.decodeX(2.0f) + numBorders) - arcRadius, this.decodeY(3.0f) - (2 * numBorders));
		this.path.lineTo((this.decodeX(1.0f) - numBorders) + arcRadius, this.decodeY(3.0f) - (2 * numBorders));
		this.path.curveTo((this.decodeX(1.0f) - numBorders) + (arcRadius / 2.0), this.decodeY(3.0f) - (2 * numBorders), this.decodeX(1.0f) - numBorders, this.decodeY(3.0f)
				- (2 * numBorders) - (arcRadius / 2.0), this.decodeX(1.0f) - numBorders, this.decodeY(3.0f) - (2 * numBorders) - arcRadius);
		this.path.lineTo(this.decodeX(1.0f) - numBorders, this.decodeY(0.0f) + numBorders + arcRadius);
		this.path.curveTo(this.decodeX(1.0f) - numBorders, this.decodeY(0.0f) + numBorders + (arcRadius / 2.0), (this.decodeX(1.0f) - numBorders) + (arcRadius / 2.0),
				this.decodeY(0.0f) + numBorders, (this.decodeX(1.0f) - numBorders) + arcRadius, this.decodeY(0.0f) + numBorders);
	...
	protected Shape decodeBorderMenuBar(JComponent c, JComponent invoker) {
		...
		if (leftPos) {
			...
			if (width < compWidth) {
				...
				this.path.lineTo((this.decodeX(2.0f) + numBorders) - arcRadius, this.decodeY(0.0f) + numBorders);
				this.path.curveTo((this.decodeX(2.0f) + numBorders) - (arcRadius / 2.0), this.decodeY(0.0f) + numBorders, this.decodeX(2.0f) + numBorders, this.decodeY(0.0f)
						+ numBorders + (arcRadius / 2.0), this.decodeX(2.0f) + numBorders, this.decodeY(0.0f) + numBorders + arcRadius);
			} else {
	...
	protected Shape decodeBorderPopupMenu(JComponent invoker, boolean rightPosition) {
		...
		if (rightPosition) {
			...
			this.path.curveTo((this.decodeX(2.0f) + numBorders) - (arcRadius / 2.0), this.decodeY(0.0f), this.decodeX(2.0f) + numBorders, this.decodeY(0.0f) + (arcRadius / 2.0),
			...
			this.path.curveTo((this.decodeX(1.0f) - numBorders) + (arcRadius / 2.0), this.decodeY(3.0f) - (2 * numBorders), this.decodeX(1.0f) - numBorders, this.decodeY(3.0f)
					- (2 * numBorders) - (arcRadius / 2.0), this.decodeX(1.0f) - numBorders, this.decodeY(3.0f) - (2 * numBorders) - arcRadius);
			this.path.lineTo(this.decodeX(1.0f) - numBorders, this.decodeY(0.0f) + numBorders + arcRadius);
			this.path.curveTo(this.decodeX(1.0f) - numBorders, this.decodeY(0.0f) + (arcRadius / 2.0), (this.decodeX(1.0f) - numBorders) + (arcRadius / 2.0), this.decodeY(0.0f),
	...
	protected Shape decodeUpperFiller(JComponent invoker) {
		...
		this.path.curveTo(this.decodeX(2.0f) - (arcRadius / 2.0), this.decodeY(0.0f) + (2 * numBorders), this.decodeX(2.0f), this.decodeY(0.0f) + (2 * numBorders)
				+ (arcRadius / 2.0), this.decodeX(2.0f), this.decodeY(0.0f) + (2 * numBorders) + arcRadius);
		...
		this.path.curveTo(this.decodeX(1.0f), this.decodeY(0.0f) + (2 * numBorders) + (arcRadius / 2.0), this.decodeX(1.0f) + (arcRadius / 2.0), this.decodeY(0.0f)
				+ (2 * numBorders), this.decodeX(1.0f) + arcRadius, this.decodeY(0.0f) + (2 * numBorders));
	...
	protected Shape decodeUpperFillerPopupMenu(JComponent invoker, boolean rightPosition) {
		...
		if (rightPosition) {
			...
			this.path.curveTo(this.decodeX(2.0f) - (arcRadius / 2.0), this.decodeY(0.0f) + numBorders, this.decodeX(2.0f), this.decodeY(0.0f) + numBorders + (arcRadius / 2.0),
			...
			this.path.curveTo(this.decodeX(1.0f), this.decodeY(0.0f) + numBorders + (arcRadius / 2.0), this.decodeX(1.0f) + (arcRadius / 2.0), this.decodeY(0.0f) + numBorders,
	...
	protected Shape decodeBottomFiller() {
		...
		this.path.curveTo(this.decodeX(2.0f), this.decodeY(3.0f) - numBorders - (arcRadius / 2.0), this.decodeX(2.0f) - (arcRadius / 2.0), this.decodeY(3.0f) - numBorders,
		...
		this.path.curveTo(this.decodeX(1.0f) + (arcRadius / 2.0), this.decodeY(3.0f) - numBorders, this.decodeX(1.0f), this.decodeY(3.0f) - numBorders - (arcRadius / 2.0),
	...
	protected com.ontimize.plaf.utils.DerivedColor getDerivedColor(Color parent, float hOffset, float sOffset, float bOffset, int aOffset) {
