 ...
 *
 * This kind of extensions and final and abstract painters were developed to allow users to have
 * several componentes which are painted in the same way, and configured by the same set of
 * properties, but having only a common code to paint (the class that this one extends).
 *
...
public class OTextAreaPainter extends AbstractRegionPainter {
    ...
    protected void paintBackgroundDisabledAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorDisabled);
    }

    protected void paintBackgroundDisabledRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y,
            int width, int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorDisabledRequired);
    }

    protected void paintBackgroundEnabledAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorEnabled);
    }

    protected void paintBackgroundRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorRequired);
    }

    protected void paintBackgroundFocusedAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorFocused);
    }

    protected void paintBackgroundFocusedRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        this.drawBackground(g, c, x, y, width, height, this.backgroundColorFocusedRequired);
    }
    ...
    protected void paintBorderDisabledRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
    ...
    protected void paintBorderFocusedRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
        if ((this.degradatedBorderColorFocused != null) && (this.degradatedBorderColorFocused.length > 0)) {
            this.drawDegradatedBorders(g, c, x, y, width, height, this.degradatedBorderColorFocused);
        }
    ...
    protected void paintBorderRequiredAndNotInScrollPane(Graphics2D g, JComponent c, int x, int y, int width,
            int height) {
    ...
    protected void defineBorderFillers(Graphics2D g, JComponent c, int x, int y) {
        ...
        Shape s = ShapeFactory.getInstace()
            .createRoundCorner(x + OTextAreaPainter.numBorders, y + OTextAreaPainter.numBorders,
                    c.getInsets().left - OTextAreaPainter.numBorders,
        ...
        s = ShapeFactory.getInstace()
            .createRoundCorner(c.getBounds().width - OTextAreaPainter.numBorders, y + OTextAreaPainter.numBorders,
                    c.getInsets().right - OTextAreaPainter.numBorders, c.getInsets().top - OTextAreaPainter.numBorders,
                    ShapeFactory.TOP_RIGHT_CORNER, false);
        g.fill(s);

        s = ShapeFactory.getInstace()
            .createRoundCorner(x + OTextAreaPainter.numBorders, c.getBounds().height - OTextAreaPainter.numBorders,
                    c.getInsets().left - OTextAreaPainter.numBorders,
                    c.getInsets().bottom - OTextAreaPainter.numBorders, ShapeFactory.BOTTOM_LEFT_CORNER, false);
        g.fill(s);

        s = ShapeFactory.getInstace()
            .createRoundCorner(c.getBounds().width - OTextAreaPainter.numBorders,
                    c.getBounds().height - OTextAreaPainter.numBorders,
                    c.getInsets().right - OTextAreaPainter.numBorders,
                    c.getInsets().bottom - OTextAreaPainter.numBorders, ShapeFactory.BOTTOM_RIGHT_CORNER, false);
        g.fill(s);

        // Drawing rectangle zones...
        // top
        s = ShapeFactory.getInstace()
            .createRectangle(insets.left, y + OTextAreaPainter.numBorders, bounds.width - insets.left - insets.right,
                    insets.top - OTextAreaPainter.numBorders);
        g.fill(s);
        // bottom
        s = ShapeFactory.getInstace()
            .createRectangle(insets.left, (y + bounds.height) - insets.bottom,
                    bounds.width - insets.left - insets.right,
                    insets.bottom - OTextAreaPainter.numBorders);
        g.fill(s);
        // left
        s = ShapeFactory.getInstace()
            .createRectangle(x + OTextAreaPainter.numBorders, insets.top,
                    insets.right - OTextAreaPainter.numBorders - 1,
                    bounds.height - insets.top - insets.bottom);
        g.fill(s);
        // right
        s = ShapeFactory.getInstace()
            .createRectangle(((x + bounds.width) - insets.right) + 1, insets.top,
                    insets.right - OTextAreaPainter.numBorders,
                    bounds.height - insets.top - insets.bottom);
        g.fill(s);
    ...
    protected void defineBorderFillersInScrollPane(Graphics2D g, JComponent c, int x, int y) {
        ...
        if (((vScrollBar != null) && vScrollBar.isVisible()) && ((hScrollBar != null) && hScrollBar.isVisible())) {
            s = ShapeFactory.getInstace()
                .createRectangle(bounds.width - insets.right, bounds.height - insets.bottom, insets.right,
                        insets.bottom);
            g.fill(s);
        } else if (!((vScrollBar != null) && vScrollBar.isVisible())
            ...
            s = this.createBottomRightCorner(bounds.width - insets.right, c.getBounds().height, c.getInsets().right,
                    c.getInsets().bottom);
            g.fill(s);
        }
        ...
        // top
        s = ShapeFactory.getInstace()
            .createRectangle(insets.left, y, bounds.width - insets.left - insets.right, insets.top);
        g.fill(s);
        // bottom
        if ((hScrollBar != null) && hScrollBar.isVisible()) {
            s = ShapeFactory.getInstace()
                .createRectangle(insets.left, (y + bounds.height) - insets.bottom,
                        bounds.width - insets.left - insets.right, insets.bottom);
        } else {
            s = ShapeFactory.getInstace()
                .createRectangle(insets.left, (y + bounds.height) - insets.bottom,
                        bounds.width - insets.left - insets.right, insets.bottom);
        ...
        // left
        s = ShapeFactory.getInstace()
            .createRectangle(x, insets.top, insets.left, bounds.height - insets.top - insets.bottom);
        g.fill(s);
        // right
        s = ShapeFactory.getInstace()
            .createRectangle((x + bounds.width) - insets.right, insets.top, insets.right,
                    bounds.height - insets.top - insets.bottom);
        g.fill(s);
    ...
    protected Shape createRoundRect(double x, double y, double w, double h, Insets insets) {
        ...
        // right bottom corner
        this.path.curveTo((x + w) - (insets.right / 2.0), y + h, x + w, (y + h) - (insets.bottom / 2.0), x + w,
                (y + h) - insets.bottom);
