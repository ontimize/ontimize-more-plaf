...
public class OComboBoxArrowButtonPainter extends AbstractRegionPainter {
	...
	protected void init() {
		...
		// required:
		obj = UIManager.getLookAndFeelDefaults().get(this.getComponentKeyName() + "[Required].foreground");
		if (obj instanceof Paint) {
			this.foregroundColorRequired = (Paint) obj;
		} else {
			this.foregroundColorRequired = this.color1;
		}
	}
