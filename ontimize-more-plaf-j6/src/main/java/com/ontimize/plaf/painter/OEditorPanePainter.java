...
public class OEditorPanePainter extends OTextAreaPainter {
    ...
    protected void drawDegradatedBordersInScrollPane(Graphics2D g, JComponent c, int x, int y, Paint[] colors) {
    	...
    	paintBorderInScrollPane(g, c, x, y, colors[0]);
    }
    ...
    protected void paintBorderInScrollPane(Graphics2D g, JComponent c, int x ,int y, Paint color){
    	...
    	if(vScrollBar!=null && vScrollBar.isVisible()){
    		...
    		g.setPaint(color);
    		s = ShapeFactory.getInstace().createRoundCorner( c.getBounds().width-numBorders, y+numBorders, c.getInsets().right-numBorders-1, c.getInsets().top-numBorders+1, ShapeFactory.TOP_RIGHT_CORNER, true);
        	g.draw(s);
    	}
    	...
    	if( (vScrollBar!=null && vScrollBar.isVisible()) ||  (hScrollBar!=null && hScrollBar.isVisible())   ){
    		...
    		g.setPaint(color);
	    	s = ShapeFactory.getInstace().createRoundCorner( c.getBounds().width-numBorders, c.getBounds().height-numBorders, c.getInsets().right-numBorders-1, c.getInsets().bottom-numBorders, ShapeFactory.BOTTOM_RIGHT_CORNER, true);
	    	g.fill(s);
    	}
    	...
    	if(hScrollBar!=null && hScrollBar.isVisible()){
    		...
    		g.setPaint(color);
    		s = ShapeFactory.getInstace().createRoundCorner( x+numBorders, c.getBounds().height-numBorders, c.getInsets().left-numBorders, c.getInsets().bottom-numBorders, ShapeFactory.BOTTOM_LEFT_CORNER, true);
        	g.draw(s);
    	}
    ...
    protected void defineBorderFillersInScrollPane(Graphics2D g, JComponent c, int x, int y) {
    	...
    	//Bottom right corner...
    	if( (vScrollBar!=null && vScrollBar.isVisible()) ||  (hScrollBar!=null && hScrollBar.isVisible())   ){
	    	s = ShapeFactory.getInstace().createRectangle( bounds.width-insets.right, bounds.height-numBorders-insets.bottom, insets.right-numBorders, c.getInsets().bottom-numBorders);
	    	g.fill(s);
    	}else{
    	...
    	//Bottom left corner...
    	if(hScrollBar!=null && hScrollBar.isVisible()){
    		s = ShapeFactory.getInstace().createRectangle( x+numBorders, c.getBounds().height-numBorders-insets.bottom, c.getInsets().left-numBorders, c.getInsets().bottom-numBorders);
        	g.fill(s);
    	}else{
		...
		g.fill(s);
		//bottom
		s = ShapeFactory.getInstace().createRectangle( insets.left-1, y+bounds.height-insets.bottom, bounds.width-insets.left-insets.right+2, insets.bottom-numBorders);
		g.fill(s);
		//left
		...
		g.fill(s);
		//right
		s = ShapeFactory.getInstace().createRectangle( x+bounds.width-insets.right+1,insets.top, insets.right-numBorders-1, bounds.height-insets.top-insets.bottom);
		g.fill(s);
