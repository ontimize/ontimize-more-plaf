...
public class OTabbedPanePainter extends AbstractRegionPainter {
    ...
    protected void doPaint(Graphics2D g, JComponent c, int width, int height, Object[] extendedCacheKeys) {
        ...
        switch(state) {
        	case BORDER_ENABLED: paintBorderEnabled(g,c,width,height); break;
        	case BORDER_DISABLED: paintBorderDisabled(g,c,width,height); break;
        }
    }
	...
	protected void init (){
		...
		// disable:
		Object obj = UIManager.getLookAndFeelDefaults().get("TabbedPane[Disabled].border");
		...
		// enable:
		obj = UIManager.getLookAndFeelDefaults().get( "TabbedPane[Enabled].border");
		if(obj instanceof Paint){
    ...
    protected void paintBorderEnabled(Graphics2D g, JComponent c, int width, int height){
    	paintBorder(g, c, width, height, borderColorEnabled);
    }

    protected void paintBorderDisabled(Graphics2D g, JComponent c, int width, int height){
    	paintBorder(g, c, width, height, borderColorDisabled);
    }
    ...
    protected void paintBorder(Graphics2D g, JComponent c, int width, int height, Paint color){
		...
		if(tabPane.getTabCount()>0 && tabPane.getUI() instanceof OTabbedPaneUI){
			...
			if(numBorders<borderWidth){
				numBorders = borderWidth + borderPadding;
			}
			...
			double y2 = decodeY(1.0f);
			double y_aux= c.getInsets().top + Math.round((tabHeight-1) /2.0);
			...
			double totalBorderWidth =  borderPadding + borderWidth;
			
			if (JTabbedPane.TOP == placement) {
				xx = decodeX(1.0f) - totalBorderWidth;
				yy = decodeY(1.0f) + tabHeight / 2.0;
				ww = decodeX(3.0f) - 1 - (tabPinsets.left - totalBorderWidth) - (tabPinsets.right - totalBorderWidth);
				hh = decodeY(3.0f) - yy - 1 - (tabPinsets.bottom - totalBorderWidth);
			}else if(JTabbedPane.LEFT == placement){
				xx = decodeY(1.0f) - totalBorderWidth;
				yy = decodeX(1.0f) + tabWidth / 2.0;
				hh = decodeX(3.0f) - 1 - yy - c.getInsets().right + totalBorderWidth;
				ww = decodeY(3.0f) - 1 - (tabPinsets.top - totalBorderWidth) - (tabPinsets.bottom - totalBorderWidth);
			}else if(JTabbedPane.RIGHT == placement){
				xx = decodeY(1.0f) - totalBorderWidth;
				yy = tabPinsets.right + tabWidth / 2.0;
				hh = decodeX(3.0f) - 1 - yy - c.getInsets().left + totalBorderWidth;
				ww = decodeY(3.0f) - 1 - (tabPinsets.top - totalBorderWidth) - (tabPinsets.bottom - totalBorderWidth);
			}else if(JTabbedPane.BOTTOM==placement){
				xx = decodeX(1.0f) - totalBorderWidth;
				yy = tabPinsets.bottom +1+ tabHeight / 2.0 ;
				ww = decodeX(3.0f) - 1 - (tabPinsets.left - totalBorderWidth) - (tabPinsets.right - totalBorderWidth);
				hh = decodeY(3.0f) - 1 - yy - (tabPinsets.top - totalBorderWidth) ;
			}
			
			//Note:
			//The border width is 4 px. If insets is greater than 4, outsider pixels are not painted.
			int indexColor = 0;
			double arcRadius_copy = arcRadius;
			for(int i=0;i<borderWidth;i++){
				 double xxx = xx + i;
				 double yyy = yy +i;
				 double www = ww -2*i;
				 double hhh = hh -2*i;
				Shape s = decodeBorderPath(xxx, yyy, www, hhh, arcRadius_copy--);
				g.setPaint(OntimizeLAFColorUtils.setAlpha((Color)color, maxAlphaBorderColor - ((maxAlphaBorderColor-minAlphaBorderColor)/borderWidth)*indexColor));
				g.draw(s);
				indexColor++;
			}
			
			if(getBackgroundColor(c)!=null){
				paintInteriorBorderFillers(g, xx, yy, ww, hh, placement, getBackgroundColor(c));
			}
    ...
    protected void paintInteriorBorderFillers(Graphics2D g, double x, double y, double w, double h, int placement, Color color){
    	g.setPaint(color);
    	double totalBorder = borderWidth+borderPadding;
    	...
    	Shape leftFiller = ShapeFactory.getInstace().createSemiRoundRect(
				x+borderWidth, y+borderWidth , borderPadding, h-totalBorder, arcRadius,	arcRadius, ShapeFactory.LEFT_ORIENTATION);
		g.fill(leftFiller);
		
		Shape rightFiller = ShapeFactory.getInstace().createSemiRoundRect(
				w+x-borderPadding-borderWidth+1, y+borderWidth , borderPadding, h-totalBorder, arcRadius,
				arcRadius, ShapeFactory.RIGHT_ORIENTATION);
		g.fill(rightFiller);
		
		Shape bottomFiller = ShapeFactory.getInstace().createRectangle(x+totalBorder, h+y-totalBorder+1, 
				w+1-2*borderWidth-2*borderPadding, borderPadding);
		g.fill(bottomFiller);
    }
