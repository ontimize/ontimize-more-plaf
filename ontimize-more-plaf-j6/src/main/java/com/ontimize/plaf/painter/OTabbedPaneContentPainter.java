...
public class OTabbedPaneContentPainter extends AbstractRegionPainter {
    ...
    protected void doPaint(Graphics2D g, JComponent c, int width, int height, Object[] extendedCacheKeys) {
        ...
        switch(state) {
        	...
        	case BACKGROUND_DISABLED: paintBackgroundDisabled(g,c,width,height); break;
        }
    }
	...
	protected void init (){
		...
		// disable:
		Object obj = UIManager.getLookAndFeelDefaults().get("TabbedPane:TabbedPaneContent[Disabled].background");
	...
	protected void paintBackground(Graphics2D g, JComponent c, Paint color) {
		...
		Paint paint = getBackgroundColor(c);
		if(paint!=null){
    ...
    protected Shape decodeBackgroundPath(double x, double y, double w, double h){
    	...
    	Shape s = ShapeFactory.getInstace().createRoundCorner(x, y+h, arcRadius, arcRadius, ShapeFactory.BOTTOM_LEFT_CORNER, true);
    	path.append(s, true);
