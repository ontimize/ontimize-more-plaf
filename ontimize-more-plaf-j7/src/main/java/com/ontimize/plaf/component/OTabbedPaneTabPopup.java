...
public class OTabbedPaneTabPopup extends JPopupMenu implements TabbedPaneTabPopup {
	 ...
	 * listeners that were added in addTabbedPaneListeners().
	 */
	@Override
	 ...
	 *
	 * @return a <code>MouseListener</code> which will be added to the tabbed
	 *         pane or null
	 */
	 ...
	 * be added to the tabbed pane model.
	 *
	 ...
	 * tabbed pane. If this method returns null then it will not be added to the
	 * tabbed pane.
	 *
	...
	/**
	 * Creates an <code>ContainerListener</code> which will be added to the
	 * tabbed pane. If this method returns null then it will not be added to the
	 * tabbed pane.
	 * <p>
	 * Subclasses may override this method to return instances of their own
	 * ContainerEvent handlers.
	 *
	...
	/**
	 * Creates the JList used in the popup to display the items in the tabbed
	 * pane model. This method is called when the UI class is created.
	 *
	...
	/**
	 * Configures the list which is used to hold the tabbed pane items in the
	...
	protected JScrollPane createScroller() {
		JScrollPane sp = new JScrollPane(this.list, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
	...
	/**
	 * Configures the popup portion of the tabbed pane. This method is called
	...
	/**
	 * A listener to be registered upon the tabbed pane (<em>not</em> its popup
	 ...
	 * box.
	 * <p>
	 * Subclasses which wish to listen to tabbed pane property changes should
	...
	private class Handler implements ContainerListener, MouseListener, MouseMotionListener, PropertyChangeListener, Serializable {
		private static final long serialVersionUID = 1L;
		...
		public void mouseReleased(MouseEvent e) {
			if (e.getSource() == OTabbedPaneTabPopup.this.list) {
				if (OTabbedPaneTabPopup.this.list.getModel().getSize() > 0) {
					...
					OTabbedPaneTabPopup.this.tabbedPane.setSelectedIndex(OTabbedPaneTabPopup.this.list.getSelectedIndex());
				}
			...
			if (OTabbedPaneTabPopup.this.isVisible()) {
				...
				if ((newEvent.getPoint().y >= r.y) && (newEvent.getPoint().y <= ((r.y + r.height) - 1))) {
					...
					if (OTabbedPaneTabPopup.this.hasEntered) {
						int directionToScroll = newEvent.getPoint().y < r.y ? OTabbedPaneTabPopup.SCROLL_UP : OTabbedPaneTabPopup.SCROLL_DOWN;
						if (OTabbedPaneTabPopup.this.isAutoScrolling && (OTabbedPaneTabPopup.this.scrollDirection != directionToScroll)) {
							OTabbedPaneTabPopup.this.stopAutoScrolling();
		...
		public void componentRemoved(ContainerEvent e) {
			...
			if (OTabbedPaneTabPopup.this.getList().getModel() instanceof DefaultListModel) {
				...
				OTabbedPaneTabPopup.this.populateListModel(((DefaultListModel) OTabbedPaneTabPopup.this.getList().getModel()));
			}
		}
	}
	...
	/**
	 * Gets the AccessibleContext associated with this OTabbedPaneTabPopup. The
	 * AccessibleContext will have its parent set to the TabbedPane.
	 *
	 ...
	 * delegates the focus to the editor (if the tabbed pane is editable) or to
	 ...
	 * to synchronize the list selection with the tabbed pane selection.
	 *
	 * @param selectedIndex
	...
	protected MouseEvent convertMouseEvent(MouseEvent e) {
		...
		MouseEvent newEvent = new MouseEvent((Component) e.getSource(), e.getID(), e.getWhen(), e.getModifiers(), convertedPoint.x, convertedPoint.y, e.getXOnScreen(),
	...
	/**
	 * Calculate the placement and size of the popup portion of the tabbed pane
	 * based on the tabbed pane location and the enclosing screen bounds. If no
	...
	private Point getPopupLocation() {
		...
		popupSize.setSize(popupSize.width - (insets.right + insets.left), this.getPopupHeightForRowCount(this.tabbedPane.getTabCount()));
		...
		Rectangle popupBounds = this.computePopupBounds(tabPaneBounds.width - insets.left - insets.right - popupSize.width, btnBounds.y, popupSize.width, popupSize.height);
