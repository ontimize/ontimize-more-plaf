...
/**
 * Class that paints the check box menu item background and the check icons depending on the element
 * status (enable, disable, mouse over, selected and combinations). Paint way can be indicated in
 * the Ontimize L&F configuration properties files or default values can be used when no custom
