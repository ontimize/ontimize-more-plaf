...
public class OTabbedPanePainter extends AbstractRegionPainter {
    ...
    protected void paintBorder(Graphics2D g, JComponent c, int width, int height, Paint color) {
        ...
        if (tabPane.getTabCount() > 0 && tabPane.getUI() instanceof OTabbedPaneUI) {
            ...
            if (SwingConstants.TOP == placement) {
                ...
                ww = this.decodeX(3.0f) - 1 - (tabPinsets.left - totalBorderWidth)
                        - (tabPinsets.right - totalBorderWidth);
                ...
                ww = this.decodeY(3.0f) - 1 - (tabPinsets.top - totalBorderWidth)
                        - (tabPinsets.bottom - totalBorderWidth);
            ...
            for (int i = 0; i < OTabbedPanePainter.borderWidth; i++) {
                ...
                g.setPaint(OntimizeLAFColorUtils
                        .setAlpha(
                                (Color) color,
                                OTabbedPanePainter.maxAlphaBorderColor
                                        - ((OTabbedPanePainter.maxAlphaBorderColor - OTabbedPanePainter.minAlphaBorderColor) / OTabbedPanePainter.borderWidth)
                                        * indexColor));
                g.draw(s);
                indexColor++;
            }
    ...
    protected void paintInteriorBorderFillers(Graphics2D g, double x, double y, double w, double h, int placement,
            Color color) {
        g.setPaint(color);
        ...
        Shape leftFiller = ShapeFactory.getInstace().createSemiRoundRect(x + OTabbedPanePainter.borderWidth,
                y + OTabbedPanePainter.borderWidth, OTabbedPanePainter.borderPadding, h - totalBorder, arcRadius,
                arcRadius, ShapeFactory.LEFT_ORIENTATION);
        g.fill(leftFiller);

        Shape rightFiller = ShapeFactory.getInstace().createSemiRoundRect(
                w + x - OTabbedPanePainter.borderPadding - OTabbedPanePainter.borderWidth + 1,
                y + OTabbedPanePainter.borderWidth, OTabbedPanePainter.borderPadding, h - totalBorder, arcRadius,
        ...
        Shape bottomFiller = ShapeFactory.getInstace().createRectangle(x + totalBorder, h + y - totalBorder + 1,
                w + 1 - 2 * OTabbedPanePainter.borderWidth - 2 * OTabbedPanePainter.borderPadding,
                OTabbedPanePainter.borderPadding);
        g.fill(bottomFiller);
    }
    ...
    protected Shape decodeBorderPath(double x, double y, double w, double h, double arcRadius) {
        ...
        Shape s = ShapeFactory.getInstace().createRoundCorner(x, y, arcRadius, arcRadius, ShapeFactory.TOP_LEFT_CORNER,
                true);
        this.path.append(s, true);
        this.path.lineTo(x, y + h - arcRadius);
        s = ShapeFactory.getInstace().createRoundCorner(x, y + h, arcRadius, arcRadius,
                ShapeFactory.BOTTOM_LEFT_CORNER, true);
        this.path.append(s, true);
        this.path.lineTo(x + w - arcRadius, y + h);
        s = ShapeFactory.getInstace().createRoundCorner(x + w, y + h, arcRadius, arcRadius,
                ShapeFactory.BOTTOM_RIGHT_CORNER, true);
        this.path.append(s, true);
        this.path.lineTo(x + w, y + arcRadius);
        s = ShapeFactory.getInstace().createRoundCorner(x + w, y, arcRadius, arcRadius, ShapeFactory.TOP_RIGHT_CORNER,
                true);
