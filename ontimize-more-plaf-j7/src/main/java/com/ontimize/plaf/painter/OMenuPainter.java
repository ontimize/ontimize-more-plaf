...
public class OMenuPainter extends AbstractRegionPainter {
    ...
    protected void init() {
        ...
        // Selected:
        Object obj = UIManager.getLookAndFeelDefaults().get(
                this.getComponentKeyName() + "[Enabled+Selected].background");
    ...
    protected void paintBackgroundEnabledAndSelected(Graphics2D g, JComponent c) {
        ...
        if (right) {
            // painting borders...
            Shape s = ShapeFactory.getInstace().createRectangle(this.decodeX(1.0f), this.decodeY(0.0f),
                    this.decodeX(2.0f), this.decodeY(3.0f));
            g.setPaint(this.borderColorEnabledSelected);
            g.fill(s);

            s = ShapeFactory.getInstace().createSemiCircle(this.decodeX(1.0f) + 1, this.decodeY(0.0f),
                    this.decodeY(3.0f), ShapeFactory.ANTICLOCKWISE, false);
            g.fill(s);
            ...
            s = ShapeFactory.getInstace().createSemiCircle(this.decodeX(1.0f) + 2, this.decodeY(1.0f),
                    this.decodeY(2.0f) - this.decodeY(1.0f), ShapeFactory.ANTICLOCKWISE, false);
            g.fill(s);
        } else {
            // painting borders...
            Shape s = ShapeFactory.getInstace().createRectangle(this.decodeX(0.0f), this.decodeY(0.0f),
                    this.decodeX(2.0f), this.decodeY(3.0f));
            g.setPaint(this.borderColorEnabledSelected);
            g.fill(s);

            s = ShapeFactory.getInstace().createSemiCircle(this.decodeX(2.0f), this.decodeY(0.0f), this.decodeY(3.0f),
            ...
            s = ShapeFactory.getInstace().createSemiCircle(this.decodeX(2.0f), this.decodeY(1.0f),
                    this.decodeY(2.0f) - this.decodeY(1.0f), ShapeFactory.CLOCKWISE, false);
            g.fill(s);
