...
public class OFormTitlePainter extends AbstractRegionPainter {
    ...
    protected void paintBorderEnabled(Graphics2D g, JComponent c, int x, int y, int width, int height) {
        ...
        shadow.quadTo(x + insets.left - arc, y + height - insets.bottom, x + insets.left, y + height - insets.bottom
                - 1);

        shadow.quadTo(x + insets.left - arc + 1, y + height - insets.bottom - 1, x + insets.left - arc + 1, y + height
                - insets.bottom - arc - 1);
        ...
        shadow.quadTo(x + width - insets.right - 1 + arc, y + insets.top, x + width - insets.right - 1 + arc, y
                + insets.top + arc);
        shadow.lineTo(x + width - insets.right - 1 + arc, y + height - insets.bottom - arc);
        shadow.quadTo(x + width - insets.right - 1 + arc, y + height - insets.bottom, x + width - insets.right - 1, y
                + height - insets.bottom - 1);

        shadow.quadTo(x + width - insets.right + arc - 2, y + height - insets.bottom - 1, x + width - insets.right
                + arc - 2, y + height - insets.bottom - arc - 1);
        shadow.lineTo(x + width - insets.right + arc - 2, y + insets.top + arc);
        shadow.quadTo(x + width - insets.right + arc - 2, y + insets.top + 1, x + width - insets.right - 1, y
                + insets.top);
