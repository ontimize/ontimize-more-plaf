...
public class OFormTitlePainter extends AbstractRegionPainter {
	...
	protected void init() {
		...
		Object obj = UIManager.getLookAndFeelDefaults().get("\"FormTitle\".background");
		if(obj instanceof Paint){
			backgroundColor = (Paint)obj;
		}else{
			backgroundColor = color1;
		}
		
		obj = UIManager.getLookAndFeelDefaults().get("\"FormTitle\".bgpaint");
		if(obj instanceof Paint){
			bgPaint = (Paint)obj;
		}else{
			bgPaint = null;
		...
		obj = UIManager.getLookAndFeelDefaults().get("\"FormTitle\".shadow");
		if(obj instanceof Paint){
			shadowColor = (Paint)obj;
		}else{
	...
	protected void doPaint(Graphics2D g, JComponent c, int width, int height,
		...
		case BACKGROUND_ENABLED:
			paintBackgroundEnabled(g, c, x, y, cwidth, cheight);
			break;
		case BORDER_ENABLED:
			paintBorderEnabled(g, c, 0, 0, width, height);
			break;
		}
		
	}

	protected void paintBackgroundEnabled(Graphics2D g, JComponent c, int x,
		...
		Shape shape = decodeFormTitleShape(c);
		g.fill(shape);
		g.setColor(oldColor);
	}

	protected void paintBorderEnabled(Graphics2D g, JComponent c, int x, int y,
	...
	protected Shape decodeFormTitleShape(JComponent c){
		...
		path.lineTo(x+w, y+h);
		path.quadTo(x +w + arc, y + h, x + w + arc , y + h - arc);	/*bottom-right corner*/
