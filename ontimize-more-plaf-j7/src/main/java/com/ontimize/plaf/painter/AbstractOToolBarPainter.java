...
public abstract class AbstractOToolBarPainter extends AbstractRegionPainter {
	...
	protected void init() {
		...
		AbstractOToolBarPainter.imgUrl = UIManager.getDefaults().get("ToolBar.backgroundImage") != null ? (String) UIManager.getDefaults().get("ToolBar.backgroundImage")
		...
		if (this.useTextureImage) {
			if (AbstractOToolBarPainter.imgUrl != null) {
				...
				if (this.bgImage != null) {
					BufferedImage bi = new BufferedImage(this.bgImage.getWidth(null), this.bgImage.getHeight(null), BufferedImage.TYPE_INT_ARGB);
	...
	protected void paintDiagramBackground(Graphics2D g, JComponent c, int width, int height) {
		...
		GradientPaint downPaint = new GradientPaint(0, height, downBackgroundColor, 0, this.decodeHeight(height), topBackgroundColor);
		Graphics2D g2D = g;
		...
		Shape s = ShapeFactory.getInstace().createSemiRoundRect(0, 0, width, this.decodeHeight(height), 5, 5, ShapeFactory.TOP_ORIENTATION);
