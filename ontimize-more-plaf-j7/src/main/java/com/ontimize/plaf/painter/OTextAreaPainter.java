...
public class OTextAreaPainter extends AbstractRegionPainter {
    ...
    protected void defineBorderFillers(Graphics2D g, JComponent c, int x, int y) {
        ...
        Shape s = ShapeFactory.getInstace().createRoundCorner(x + OTextAreaPainter.numBorders,
                y + OTextAreaPainter.numBorders, c.getInsets().left - OTextAreaPainter.numBorders,
                c.getInsets().top - OTextAreaPainter.numBorders, ShapeFactory.TOP_LEFT_CORNER, false);
        g.setPaint(this.getPaintForFiller(c, s));
        g.fill(s);

        s = ShapeFactory.getInstace().createRoundCorner(c.getBounds().width - OTextAreaPainter.numBorders,
                y + OTextAreaPainter.numBorders, c.getInsets().right - OTextAreaPainter.numBorders,
                c.getInsets().top - OTextAreaPainter.numBorders, ShapeFactory.TOP_RIGHT_CORNER, false);
        g.fill(s);

        s = ShapeFactory.getInstace().createRoundCorner(x + OTextAreaPainter.numBorders,
                c.getBounds().height - OTextAreaPainter.numBorders, c.getInsets().left - OTextAreaPainter.numBorders,
                c.getInsets().bottom - OTextAreaPainter.numBorders, ShapeFactory.BOTTOM_LEFT_CORNER, false);
        g.fill(s);

        s = ShapeFactory.getInstace().createRoundCorner(c.getBounds().width - OTextAreaPainter.numBorders,
                c.getBounds().height - OTextAreaPainter.numBorders, c.getInsets().right - OTextAreaPainter.numBorders,
                c.getInsets().bottom - OTextAreaPainter.numBorders, ShapeFactory.BOTTOM_RIGHT_CORNER, false);
        g.fill(s);

        // Drawing rectangle zones...
        // top
        s = ShapeFactory.getInstace().createRectangle(insets.left, y + OTextAreaPainter.numBorders,
                bounds.width - insets.left - insets.right, insets.top - OTextAreaPainter.numBorders);
        g.fill(s);
        // bottom
        s = ShapeFactory.getInstace().createRectangle(insets.left, y + bounds.height - insets.bottom,
                bounds.width - insets.left - insets.right, insets.bottom - OTextAreaPainter.numBorders);
        g.fill(s);
        // left
        s = ShapeFactory.getInstace().createRectangle(x + OTextAreaPainter.numBorders, insets.top,
                insets.right - OTextAreaPainter.numBorders - 1, bounds.height - insets.top - insets.bottom);
        g.fill(s);
        // right
        s = ShapeFactory.getInstace().createRectangle(x + bounds.width - insets.right + 1, insets.top,
                insets.right - OTextAreaPainter.numBorders, bounds.height - insets.top - insets.bottom);
        g.fill(s);
    ...
    protected void defineBorderFillersInScrollPane(Graphics2D g, JComponent c, int x, int y) {
        ...
        if ((vScrollBar != null && vScrollBar.isVisible()) && (hScrollBar != null && hScrollBar.isVisible())) {
            s = ShapeFactory.getInstace().createRectangle(bounds.width - insets.right, bounds.height - insets.bottom,
        ...
        // Drawing rectangle zones...
        // top
        s = ShapeFactory.getInstace().createRectangle(insets.left, y, bounds.width - insets.left - insets.right,
                insets.top);
        g.fill(s);
        // bottom
        if (hScrollBar != null && hScrollBar.isVisible()) {
        ...
        g.fill(s);
        // left
        s = ShapeFactory.getInstace().createRectangle(x, insets.top, insets.left,
                bounds.height - insets.top - insets.bottom);
        g.fill(s);
        // right
        s = ShapeFactory.getInstace().createRectangle(x + bounds.width - insets.right, insets.top, insets.right,
                bounds.height - insets.top - insets.bottom);
        g.fill(s);
    ...
    protected Shape createRoundRect(double x, double y, double w, double h, Insets insets) {
        ...
        // right bottom corner
        this.path.curveTo(x + w - insets.right / 2.0, y + h, x + w, y + h - insets.bottom / 2.0, x + w, y + h
                - insets.bottom);
    ...
    protected Shape decodeBorderPath(double x, double y, double w, double h, int borderIndex) {
        ...
        this.path.moveTo(this.intValue(x), y);
        this.path.lineTo(x_arc, y);
        this.path.curveTo(x_arc - radius / 2.0, y, x_arc - radius, y + radius / 2.0, x_arc - radius - borderIndex, y
                + radius);
        this.path.lineTo(x_arc - radius - borderIndex, y + h - radius);
        this.path
                .curveTo(x_arc - radius - borderIndex, y + h - radius / 2.0, x_arc - radius / 2.0, y + h, x_arc, y + h);
        ...
        this.path.lineTo(x_arc, y + h);
        this.path.curveTo(x_arc + radius / 2.0, y + h, x_arc + radius, y + h - radius / 2.0, x_arc + radius
                + borderIndex, y + h - radius);
