...
public class OToolBarSeparatorPainter extends AbstractRegionPainter {
	...
	//of OToolBarSeparatorPainter to determine which region/state is being painted
	//by that instance.
	...
	//by a particular instance of OToolBarSeparatorPainter would be created. For the moment at least,
	...
	protected void init (){
		...
		// enable:
		Object obj = UIManager.getLookAndFeelDefaults().get( "ToolBarSeparator[Enabled].background");
	...
	protected void paintBackgroundEnabled(Graphics2D g, JComponent c) {
		//NOTE:
		//It is necessary to take into consideration that into OntimizeSynthPainterImpl
		...
		if(c!=null){
			...
			if( ((JSeparator)c).getOrientation() == SwingConstants.VERTICAL){
				length = bounds.height - 2*marginOffset;
				x = in.top + marginOffset;
				y = bounds.width/2;
				slashWidth = length;
				slashHeight = 1;	
			}else{
				length = bounds.width - 2*marginOffset;
				x = in.left + marginOffset;
				y = bounds.height/2;
				slashWidth = length;
				slashHeight = 1;
			}
			...
			rect.setRect(x, y, slashWidth, slashHeight);
			g.fill(rect);
