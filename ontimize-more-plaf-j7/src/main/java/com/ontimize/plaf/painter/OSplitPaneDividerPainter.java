...
public class OSplitPaneDividerPainter extends AbstractRegionPainter {
    ...
    protected void init() {
        ...
        obj = UIManager.getLookAndFeelDefaults()
                .get(this.getComponentKeyName() + "[Enabled+Vertical].foregroundBorder");
        if (obj instanceof Paint) {
    ...
    protected void paintForegroundEnabled(Graphics2D g, int width, int height) {
        ...
        Shape s = this.decodeForeground(x + 1, y + 1, OSplitPaneDividerPainter.buttonWidth,
                OSplitPaneDividerPainter.buttonHeight);
    ...
    protected void paintForegroundEnabledAndVertical(Graphics2D g, int width, int height) {
        ...
        Shape s = this.decodeForegroundVertical(x + 1, y + 1, OSplitPaneDividerPainter.buttonHeight,
                OSplitPaneDividerPainter.buttonWidth);
        g.setPaint(this.foregroundBorderColorEnabledVertical);
        g.fill(s);

        s = this.decodeForegroundVertical(x, y, OSplitPaneDividerPainter.buttonHeight,
                OSplitPaneDividerPainter.buttonWidth);
