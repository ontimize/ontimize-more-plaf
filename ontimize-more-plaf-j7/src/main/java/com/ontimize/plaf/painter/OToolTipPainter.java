...
public class OToolTipPainter extends AbstractRegionPainter {
    ...
    protected void paintBackgroundEnabled(Graphics2D g) {
        ...
        Shape s = this.decodeRoundRect(this.decodeX(0.0f) + OToolTipPainter.defaultShadowWidth, this.decodeY(0.0f)
                + OToolTipPainter.defaultShadowWidth, this.decodeX(3.0f) - 1 - 2 * OToolTipPainter.defaultShadowWidth,
                this.decodeY(3.0f) - 1 - 2 * OToolTipPainter.defaultShadowWidth, 5, 5);
        g.setPaint(this.borderColorEnabled);
        g.draw(s);

        s = this.decodeRoundRect(this.decodeX(0.0f) + OToolTipPainter.defaultShadowWidth + 1, this.decodeY(0.0f)
                + OToolTipPainter.defaultShadowWidth + 1, this.decodeX(3.0f) - 2 - 2
                * OToolTipPainter.defaultShadowWidth, this.decodeY(3.0f) - 2 - 2 * OToolTipPainter.defaultShadowWidth,
                5, 5);
    ...
    protected Shape decodeShadow(int x, int y) {
        ...
        this.path.curveTo(this.decodeX(3.0f) - x - 1 - arcRadius / 2.0, this.decodeY(0.0f) + yOffset + y,
                this.decodeX(3.0f) - x - 1, this.decodeY(0.0f) + yOffset - 1 + arcRadius / 2.0, this.decodeX(3.0f) - x
                        - 1, this.decodeY(0.0f) + yOffset - 1 + arcRadius);

        this.path.lineTo(this.decodeX(3.0f) - x - 1, this.decodeY(3.0f) - arcRadius - 3);
        // Bottom right corner...
        this.path.curveTo(this.decodeX(3.0f) - x, this.decodeY(3.0f) - y - 1 - arcRadius / 2.0, this.decodeX(3.0f) - 2
                - arcRadius / 2.0, this.decodeY(3.0f) - y - 1, this.decodeX(3.0f) - 2 - arcRadius, this.decodeY(3.0f)
                - y - 1);
        ...
        this.path.curveTo(this.decodeX(0.0f) + xOffset - 1 + x + arcRadius / 2.0, this.decodeY(3.0f) - y,
                this.decodeX(0.0f) + xOffset + x, this.decodeY(3.0f) - 3 - arcRadius / 2.0, this.decodeX(0.0f)
                        + xOffset + x, this.decodeY(3.0f) - 3 - arcRadius);
        ...
        this.path.curveTo(this.decodeX(0.0f) + xOffset + x, this.decodeY(0.0f) + yOffset - 1 + arcRadius / 2.0,
                this.decodeX(0.0f) + xOffset + 1 + arcRadius / 2.0, this.decodeY(0.0f) + yOffset + y,
                this.decodeX(0.0f) + xOffset + 1 + arcRadius, this.decodeY(0.0f) + yOffset + y);
