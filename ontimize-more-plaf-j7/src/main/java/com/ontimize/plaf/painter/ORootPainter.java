...
public class ORootPainter extends AbstractRegionPainter {
    ...
    protected Color frameInnerHighlightInactive;// =
                                                // decodeColor("frameInnerHighlightInactive");
    protected Color frameInnerHighlightActive;// =
                                              // decodeColor("frameInnerHighlightActive");
    ...
    protected Color frameBorderActive = this.frameBorderBase;
    protected Color frameBorderInactive = this.frameBorderBase;

    protected Color frameTopActive;// = deriveColor(frameBaseActive, 0.005208f,
                                   // -0.080105f, 0.043137f, 0);
    protected Color frameUpperMidActive;// = frameBaseActive;
    protected Color frameLowerMidActive;// = frameBaseActive;
    protected Color frameBottomActive;// = deriveColor(frameBaseActive, 0f,
                                      // 0.025723f, -0.015686f, 0);
    protected Color frameTopInactive;// = deriveColor(frameBaseInactive, 0f, 0f,
                                     // 0.050980f, 0);
    protected Color frameUpperMidInactive;// = frameBaseInactive;
    protected Color frameLowerMidInactive;// = frameBaseInactive;
    protected Color frameBottomInactive;// = deriveColor(frameBaseInactive, 0f,
                                        // 0f, -0.050980f, 0);

    // protected FourColors frameActive = new FourColors(frameTopActive,
    // frameUpperMidActive, frameLowerMidActive,
    // frameBottomActive);
    // protected FourColors frameInactive = new FourColors(frameTopInactive,
    // frameUpperMidInactive, frameLowerMidInactive,
    // frameBottomInactive);
    ...
    protected void doPaint(Graphics2D g, JComponent c, int width, int height, Object[] extendedCacheKeys) {
        if (this.state == ORootPainter.BACKGROUND_ENABLED_NOFRAME) {
            return;
        }

        // Shape s = shapeGenerator.createRoundRectangle(0, 0, (width - 1),
        // (height - 1), CornerSize.FRAME_BORDER,
        ...
        if (cArray != null) {
            ...
            for (Component comp : cArray) {
                ...
                if (comp instanceof JToolBar) {
                    ...
                    if (ORootPainter.toolBarNorthState.isInState((JComponent) comp)) {
                        topToolBarHeight = comp.getHeight();
                    } else if (ORootPainter.toolBarSouthState.isInState((JComponent) comp)) {
        ...
        int titleHeight = ORootPainter.TITLE_BAR_HEIGHT;
        ...
        // if (c.getClientProperty(OntimizeRootPaneUI.UNIFIED_TOOLBAR_LOOK) ==
        // Boolean.TRUE) {
        // // Draw background gradient.
        // s = shapeGenerator.createRoundRectangle(1, 1, width - 2, height - 2,
        // CornerSize.FRAME_INNER_HIGHLIGHT,
        // CornerStyle.ROUNDED, CornerStyle.SQUARE, CornerStyle.SQUARE,
        // CornerStyle.ROUNDED);
        // g.setPaint(getFrameInteriorPaint(s, titleHeight, topToolBarHeight,
        // bottomToolBarHeight));
        // g.fill(s);
        // } else {
        // // Paint title bar.
        // s = shapeGenerator.createRoundRectangle(1, 1, width - 2, titleHeight,
        // CornerSize.FRAME_INNER_HIGHLIGHT,
        // CornerStyle.ROUNDED, CornerStyle.SQUARE, CornerStyle.SQUARE,
        // CornerStyle.ROUNDED);
        ...
        // g.fill(s);
        // // Paint contents.
        // s = shapeGenerator.createRoundRectangle(1, titleHeight, width - 2,
        // height - titleHeight - 2, CornerSize.FRAME_INNER_HIGHLIGHT,
        // CornerStyle.SQUARE, CornerStyle.SQUARE, CornerStyle.SQUARE,
        // CornerStyle.SQUARE);
        ...
        // }
        //
        // s = shapeGenerator.createRoundRectangle(1, 1, width - 3, height - 3,
        // CornerSize.FRAME_INTERIOR);
     ...
     *
     * @param s
     *            the frame interior shape.
     * @param titleHeight
     *            the height of the title portion.
     * @param topToolBarHeight
     *            the height of the top toolbar, or 0 if none.
     * @param bottomToolBarHeight
     *            the height of the bottom toolbar, or 0 if none.
     *
     * @return the paint.
     */
    // public Paint getFrameInteriorPaint(Shape s, int titleHeight, int
    // topToolBarHeight, int bottomToolBarHeight) {
    // return createFrameGradient(s, titleHeight, topToolBarHeight,
    // bottomToolBarHeight, getFrameInteriorColors());
    // }
     ...
     *            the interior shape.
     * @param titleHeight
     *            the height of the title bar, or 0 if none.
     * @param topToolBarHeight
     *            the height of the top toolbar, or 0 if none.
     * @param bottomToolBarHeight
     *            the height of the bottom toolbar, or 0 if none.
     * @param defColors
     *            the color set to construct the gradient from.
     *
     * @return the gradient.
     */
    // protected Paint createFrameGradient(Shape s, int titleHeight, int
    // topToolBarHeight, int bottomToolBarHeight, FourColors defColors) {
    ...
    // };
    // colors = new Color[] { defColors.top, defColors.upperMid,
    // defColors.lowerMid, defColors.bottom };
     ...
     * @return the gradient.
     */
    // protected Paint createTitleBarGradient(Shape s, int titleHeight,
    // FourColors defColors) {
    // Rectangle2D bounds = s.getBounds2D();
