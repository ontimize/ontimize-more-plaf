...
public abstract class AbstractRegionPainter implements Painter<JComponent> {
	...
	/**
	 * Added to be able to get the whole area that must be painted, in order to create Ontimize Painters for the whole area.
	 * Sometimes, only a part of the whole area will be painted (depending on the specifications in the classes that extends this one).
	 * @param path
	 * @return
	 */
	 ...
	 */
	protected Color decodeColor(String key, float hOffset, float sOffset,
	 ...
	 */
	protected Color decodeColor(Color color1, Color color2,
			float midPoint) {
		if (UIManager.getLookAndFeel() instanceof NimbusLookAndFeel){
			NimbusLookAndFeel laf = (NimbusLookAndFeel) UIManager.getLookAndFeel();
	 ...
	 */
	protected Color getComponentColor(JComponent c, String property,
			Color defaultColor,
	...
	protected void paintWith9SquareCaching(Graphics2D g, PaintContext ctx,
	...
	protected void paintWithFixedSizeCaching(Graphics2D g, JComponent c, int w,
	...
	protected VolatileImage getImage(GraphicsConfiguration config, JComponent c,
	...
	protected void paint0(Graphics2D g, JComponent c, int width, int height,
			Object[] extendedCacheKeys) {
     ...
     */
    protected Color getDerivedColor(Color color1, Color color2,
