...
public class OTreeCellRendererPainter extends AbstractRegionPainter {
    ...
    protected void paintBackground(Graphics2D g, JComponent c, int width, int height, boolean selected, boolean leaf,
        ...
        // left
        Shape s = ShapeFactory.getInstace().createSemiCircle(this.getLabelStart((JLabel) c) + offset, 1, height - 2,
                ShapeFactory.ANTICLOCKWISE, false);
        g.fill(s);

        // center
        g.fillRect(this.getLabelStart((JLabel) c) + offset, 1, width - ((offset * 2) + this.getLabelStart((JLabel) c))
                - 1, height - 2);
        ...
        s = ShapeFactory.getInstace()
                .createSemiCircle(
                        this.getLabelStart((JLabel) c) + offset
                                + (width - ((offset * 2) + this.getLabelStart((JLabel) c))) - 1, 1, height - 2,
