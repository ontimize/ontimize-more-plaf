...
public class OComboBoxUI extends BasicComboBoxUI implements PropertyChangeListener, SynthUI {
    ...
    /**
     * Used to adjust the location and size of the popup. Very useful for situations such as we find in
     * Nimbus where part of the border is used to paint the focus. In such cases, the border is empty
     * space, and not part of the "visual" border, and in these cases, you'd like the popup to be
     * adjusted such that it looks as if it were next to the visual border. You may want to use negative
     * insets to get the right look.
     */
    public Insets popupInsets;

    /**
     * This flag may be set via UIDefaults. By default, it is false, to preserve backwards
     * compatibility. If true, then the combo will "act as a button" when it is not editable.
     */
    protected boolean buttonWhenNotEditable;

    /**
     * A flag to indicate that the combo box and combo box button should remain in the PRESSED state
     * while the combo popup is visible.
     */
    protected boolean pressedWhenPopupVisible;

    /**
     * When buttonWhenNotEditable is true, this field is used to help make the combo box appear and
     * function as a button when the combo box is not editable. In such a state, you can click anywhere
     * on the button to get it to open the popup. Also, anywhere you hover over the combo will cause the
     * entire combo to go into "rollover" state, and anywhere you press will go into "pressed" state.
     * This also keeps in sync the state of the combo and the arrowButton.
     */
    ...
    /**
     * If true, then the cell renderer will be forced to be non-opaque when used for rendering the
     * selected item in the combo box (not in the list), and forced to opaque after rendering the
     * selected value.
     */
    protected boolean forceOpaque = false;

    /**
     * NOTE: This serves the same purpose as the same field in BasicComboBoxUI. It is here because I
     * could not give the padding field in BasicComboBoxUI protected access in an update release.
     */
    ...
    public void update(Graphics g, JComponent c) {
        ...
        ContextUtils.getPainter(context)
            .paintComboBoxBackground(context, g, 0, 0,
    ...
    /**
     * @return true if this combo box should act as one big button. Typically only happens when
     *         buttonWhenNotEditable is true, and comboBox.isEditable is false.
     */
    ...
    /**
     * Return the default size of an empty display area of the combo box using the current renderer and
     * font.
     *
     * This method was overridden to use SynthComboBoxRenderer instead of DefaultListCellRenderer as the
     * default renderer when calculating the size of the combo box. This is used in the case of the
     * combo not having any data.
    ...
    /**
     * This has been refactored out in hopes that it may be investigated and simplified for the next
     * major release. adding/removing the component to the currentValuePane and changing the font may be
     * redundant operations.
     *
     * NOTE: This method was copied in its entirety from BasicComboBoxUI. Might want to make it
     * protected in BasicComboBoxUI in Java 7
     */
     ...
     *
     * Be aware that SynthFileChooserUIImpl relies on the fact that the default renderer installed on a
     * Synth combo box is a JLabel. If this is changed, then an assert will fail in
    ...
    /**
     * Handles all the logic for treating the combo as a button when it is not editable, and when
     * shouldActLikeButton() is true. This class is a special ButtonModel, and installed on the
     * arrowButton when appropriate. It also is installed as a mouse listener and mouse motion listener
     * on the combo box. In this way, the state between the button and combo are in sync. Whenever one
     * is "over" both are. Whenever one is pressed, both are.
     */
    protected class ButtonHandler extends DefaultButtonModel
            implements MouseListener, PopupMenuListener {

        /**
         * Indicates that the mouse is over the combo or the arrow button. This field only has meaning if
         * buttonWhenNotEnabled is true.
         */
        protected boolean over;

        /**
         * Indicates that the combo or arrow button has been pressed. This field only has meaning if
         * buttonWhenNotEnabled is true.
         */
        ...
        /**
         * <p>
         * Updates the internal "pressed" state. If shouldActLikeButton() is true, and if this method call
         * will change the internal state, then the combo and button will be repainted.
         * </p>
         *
         * <p>
         * Note that this method is called either when a press event occurs on the combo box, or on the
         * arrow button.
         * </p>
         */
        ...
        /**
         * <p>
         * Updates the internal "over" state. If shouldActLikeButton() is true, and if this method call will
         * change the internal state, then the combo and button will be repainted.
         * </p>
         *
         * <p>
         * Note that this method is called either when a mouseover/mouseoff event occurs on the combo box,
         * or on the arrow button.
         * </p>
         */
         ...
         * {@inheritDoc}
         *
         * Ensures that isPressed() will return true if the combo is pressed, or the arrowButton is pressed,
         * <em>or</em> if the combo popup is visible. This is the case because a combo box looks pressed
         * when the popup is visible, and so should the arrow button.
         */
        @Override
         ...
         * {@inheritDoc}
         *
         * Ensures that the armed state is in sync with the pressed state if shouldActLikeButton is true.
         * Without this method, the arrow button will not look pressed when the popup is open, regardless of
         * the result of isPressed() alone.
         */
        @Override
         ...
         * {@inheritDoc}
         *
         * Ensures that isRollover() will return true if the combo is rolled over, or the arrowButton is
         * rolled over.
         */
        @Override
         ...
         * @inheritDoc
         *
         *             Ensures that the combo box is repainted when the popup is closed. This avoids a bug
         *             where clicking off the combo wasn't causing a repaint, and thus the combo box still
         *             looked pressed even when it was not.
         *
         *             This bug was only noticed when acting as a button, but may be generally present. If
         *             so, remove the if() block
         */
        @Override
