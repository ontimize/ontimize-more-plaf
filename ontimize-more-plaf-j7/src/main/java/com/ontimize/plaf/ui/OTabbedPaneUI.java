    ...
    protected void updateStyle(JTabbedPane c) {
        ...
        if (o != null && o instanceof OTabCloseListener) {
            if (tabCloseListener == null) {
                tabCloseListener = (OTabCloseListener) o;
            }
        }
...
//        tabContentStyle.uninstallDefaults(tabContentContext);
    ...
    protected void paintScrollButtonBackground(SynthContext ss, Graphics g, JButton scrollButton) {
        ...
        if(buttonState == ENABLED && model.isRollover()){
    ...
    protected void layoutLabel(SynthContext ss, int tabPlacement, FontMetrics metrics, int tabIndex, String title, Icon icon,
        ...
        if (iconRect.width > 0) {
            ...
            textRect.x = xx - textRect.width + iconRect.width + textIconGap;
        }
    }
    ...
    protected int calculateTabWidth(int tabPlacement, int tabIndex, FontMetrics metrics) {
        ...
        //Adding 20 px (10 per each side) to set a margin between the text and the end of the tab.
        width = width + 20;
        return width;
    }
    ...
    public Rectangle getMaxTabBounds(JTabbedPane pane){
        ...
        for(int i=0;i<size;i++){
            ...
            /*
             * Note:
             * Because of scroll buttons bounds does not update properly and depending on the
             * position of them and if they are displaying or not, when requesting dimension of tab bounds
    ...
    protected SynthContext getTabContext(int index, boolean selected, boolean isMouseDown, boolean isMouseOver, boolean hasFocus) {
        ...
        if (!tabPane.isEnabled() || !tabPane.isEnabledAt(index)) {
            ...
            state =  OntimizeLookAndFeel.getComponentState(tabPane);
