...
public class OTableUI extends BasicTableUI implements SynthUI, PropertyChangeListener, ViewportPainter {
    ...
    public void paintViewportBackground(SynthContext context, Graphics g, JComponent c, int width, int height,
            int top) {
    ...
    protected void paintCell(SynthContext context, Graphics g, Rectangle cellRect, int row, int column) {
        if (table.isEditing() && table.getEditingRow() == row && table.getEditingColumn() == column) {
            ...
            if (component instanceof JComponent && useUIBorder) {
                ...
                if (border != null) {
                    if (component instanceof JComboBox) {
                        if (border instanceof TableCellEditorBorder
                                && ((TableCellEditorBorder) border).getDelegateBorder() instanceof CompoundBorder) {
                            Border b = ((CompoundBorder) ((TableCellEditorBorder) border).getDelegateBorder())
                                .getOutsideBorder();
            ...
            rendererPane.paintComponent(g, component, table, cellRect.x, cellRect.y,
                    table.getShowVerticalLines() ? cellRect.width - 1 : cellRect.width,
    ...
    /**
     * Creates a custom {@link CellRendererPane} that sets the renderer component to be non-opaque if
     * the associated row isn't selected. This custom {@code CellRendererPane} is needed because a table
     * UI delegate has no prepare renderer like {@link JTable} has.
     * @return DOCUMENT ME!
     */
    protected CellRendererPane createCustomCellRendererPane() {
        return new CellRendererPane() {
            @Override
            public void paintComponent(Graphics graphics, Component component, Container container, int x, int y, int w,
                    int h,
