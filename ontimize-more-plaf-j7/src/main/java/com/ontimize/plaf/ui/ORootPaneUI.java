...
public class ORootPaneUI extends BasicRootPaneUI implements SynthUI {
    ...
    /**
     * A custom layout manager that is responsible for the layout of layeredPane, glassPane, menuBar and
     * titlePane, if one has been installed.
     */
    ...
    protected static class OntimizeRootLayout implements LayoutManager2 {
        ...
        public Dimension preferredLayoutSize(Container container) {
            ...
            return new Dimension(Math.max(Math.max(cpWidth, mbWidth), tpWidth) + insets.left + insets.right,
                    cpHeight + mbHeight + tpWidth + insets.top
                            + insets.bottom);
        }
    ...
    protected class MouseInputHandler_ implements MouseInputListener {
        ...
        public void mousePressed(MouseEvent mouseevent) {
            ...
            if ((i == 0) && (ORootPaneUI.this.titlePane != null) && ORootPaneUI.this.titlePane.contains(point1)
                ...
                this.dragDimension = new Dimension(ORootPaneUI.this.window.getWidth(),
                        ORootPaneUI.this.window.getHeight());
        ...
        public void mouseDragged(MouseEvent mouseevent) {
            ...
            if (this.windowAction == MouseInputHandler_.WINDOW_MOVE) {
                ...
                Point offset = new Point((int) current.getX() - (int) this.start_drag.getX(),
                        (int) current.getY() - (int) this.start_drag.getY());
                ...
                if ((this.resizeType == MouseInputHandler_.RIGHT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.TOP_RIGHT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.DOWN_RIGHT_RESIZE)) {
                ...
                if ((this.resizeType == MouseInputHandler_.DOWN_RESIZE)
                        || (this.resizeType == MouseInputHandler_.DOWN_LEFT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.DOWN_RIGHT_RESIZE)) {
                    // rectangle.height =
                ...
                if ((this.resizeType == MouseInputHandler_.TOP_RESIZE)
                        || (this.resizeType == MouseInputHandler_.TOP_LEFT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.TOP_RIGHT_RESIZE)) {
                ...
                if ((this.resizeType == MouseInputHandler_.LEFT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.TOP_LEFT_RESIZE)
                        || (this.resizeType == MouseInputHandler_.DOWN_LEFT_RESIZE)) {
        ...
        protected boolean isFrameResizable() {
            return (this.frame != null) && this.frame.isResizable()
                    && ((this.frame.getExtendedState() & Frame.MAXIMIZED_BOTH) == 0);
        }
    ...
    /**
     * Maps from positions to cursor type. Refer to calculateCorner and calculatePosition for details of
     * this.
     */
    protected static final int[] cursorMapping = new int[] { Cursor.NW_RESIZE_CURSOR, Cursor.NW_RESIZE_CURSOR,
    ...
    protected class MouseInputHandler implements MouseInputListener {
        ...
        /**
         * Returns the corner that contains the point <code>x</code>, <code>y</code>, or -1 if the position
         * doesn't match a corner.
         */
        ...
        /**
         * Returns the Cursor to render for the specified corner. This returns 0 if the corner doesn't map
         * to a valid Cursor
         */
        ...
        /**
         * Returns an integer indicating the position of <code>spot</code> in <code>width</code>. The return
         * value will be: 0 if < BORDER_DRAG_THICKNESS 1 if < CORNER_DRAG_WIDTH 2 if >= CORNER_DRAG_WIDTH &&
         * < width - BORDER_DRAG_THICKNESS 3 if >= width - CORNER_DRAG_WIDTH 4 if >= width -
