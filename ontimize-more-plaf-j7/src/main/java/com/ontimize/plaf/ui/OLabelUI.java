...
public class OLabelUI extends BasicLabelUI implements SynthUI {
    ...
    protected void paint(SynthContext context, Graphics g) {
        ...
        if (label instanceof BasicTreeCellRenderer){
        	...
        	if (!(renderer.isLeaf() || renderer.isRoot())){
        		...
        		int size = drawBasicTreeCellRendererChildCount((Graphics2D)g,(BasicTreeCellRenderer)label);
        ...
        if(label instanceof VisualCalendarComponent.HeaderRenderer){
        	...
        	context.getStyle().getGraphicsUtils(context).paintText(context, g, label.getText(), null, label.getHorizontalAlignment(),
                    label.getVerticalAlignment(), label.getHorizontalTextPosition(), label.getVerticalTextPosition(),
                    label.getIconTextGap(), label.getDisplayedMnemonicIndex(), 1);
    ...
    protected int drawBasicTreeCellRendererChildCount(Graphics2D g,BasicTreeCellRenderer renderer){
    	...
    	if(Tree.enabledRowCount==false) return 0;
...
//    	FontMetrics fm = SwingUtilities2.getFontMetrics(renderer, g);
    	...
    	if (w < (width+ getLabelStar(renderer))){
    		return 0;
    	}
...
//        	System.out.println("LabelUI Minimum size :" + size);
        return size;
    }
