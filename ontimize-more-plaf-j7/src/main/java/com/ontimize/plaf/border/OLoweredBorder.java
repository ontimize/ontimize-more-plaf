...
public class OLoweredBorder extends AbstractRegionPainter implements Border {
    ...
    public OLoweredBorder() {
        this.paintContext = new PaintContext(OLoweredBorder.DEFAULT_INSETS, new Dimension(
                OLoweredBorder.DEFAULT_IMG_SIZE, OLoweredBorder.DEFAULT_IMG_SIZE), false,
    ...
    public OLoweredBorder(Insets insets, Integer titleSize, Integer radius) {
        ...
        int size = titleSize != null ? titleSize.intValue() : OLoweredBorder.DEFAULT_IMG_SIZE;
        this.radius = radius != null ? radius.intValue() : OLoweredBorder.DEFAULT_RADIUS;

        this.paintContext = new PaintContext(this.insets, new Dimension(size, size), false,
                PaintContext.CacheMode.NO_CACHING, Integer.MAX_VALUE, Integer.MAX_VALUE);
    }
    ...
    protected void paint_(Graphics2D g, JComponent c, int width, int height, Object[] extendedCacheKeys) {
        ...
        // TOP
        Shape shape = ShapeFactory.getInstace().createSemiRoundRect(1, 1, width - 2, insets.top - 1, this.radius,
                this.radius, ShapeFactory.TOP_ORIENTATION);
        g.fill(shape);
        // LEFT
        // g.fillRect(1, insets.top , insets.left -1, height - insets.top -
        // insets.bottom );
        shape = ShapeFactory.getInstace().createRectangle(1, insets.top, insets.left - 1,
                height - insets.top - insets.bottom);
        g.fill(shape);
        // RIGHT
        // g.fillRect(width-insets.right , insets.top, insets.right - 1,height -
        // insets.top - insets.bottom );
        shape = ShapeFactory.getInstace().createRectangle(width - insets.right, insets.top, insets.right - 1,
                height - insets.top - insets.bottom);
        g.fill(shape);
        // BOTTOM
        shape = ShapeFactory.getInstace().createSemiRoundRect(1, height - insets.bottom, width - 2, insets.bottom - 1,
                this.radius, this.radius, ShapeFactory.BOTTOM_ORIENTATION);
        g.fill(shape);
        g.setColor(oldColor);
    }
    ...
    protected Shape getTopShape(JComponent c, LinearGradient lg) {
        ...
        if (lg != null && GradientDirection.HORIZONTAL_DIRECTION == lg.getDirectionType()) {
            shape = ShapeFactory.getInstace().createRectangle(insets.left, 1, width - insets.left - insets.right,
                    insets.top - 1);
        } else {
            shape = ShapeFactory.getInstace().createSemiRoundRect(1, 1, width - 2, insets.top - 1, this.radius,
                    this.radius, ShapeFactory.TOP_ORIENTATION);
        }
        return shape;
    }
    ...
    protected Shape getBottomShape(JComponent c, LinearGradient lg) {
        ...
        Shape shape = null;
        if (lg != null && GradientDirection.HORIZONTAL_DIRECTION == lg.getDirectionType()) {
            shape = ShapeFactory.getInstace().createRectangle(insets.left, height - insets.bottom,
                    width - insets.left - insets.right, insets.bottom - 1);
        } else {
            shape = ShapeFactory.getInstace().createSemiRoundRect(1, height - insets.bottom, width - 2,
                    insets.bottom - 1, this.radius, this.radius, ShapeFactory.BOTTOM_ORIENTATION);
        }
        return shape;
    }
    ...
    protected Shape getLeftShape(JComponent c, LinearGradient lg) {
        ...
        Shape shape = null;
        if (lg != null && GradientDirection.HORIZONTAL_DIRECTION == lg.getDirectionType()) {
            shape = ShapeFactory.getInstace().createSemiRoundRect(1, 1, insets.left - 1, height - 2, this.radius,
                    this.radius, ShapeFactory.LEFT_ORIENTATION);
        } else {
            shape = ShapeFactory.getInstace().createRectangle(1, insets.top, insets.left - 1,
                    height - insets.top - insets.bottom);
        }
    ...
    protected Shape getRightShape(JComponent c, LinearGradient lg) {
        ...
        Shape shape = null;
        if (lg != null && GradientDirection.HORIZONTAL_DIRECTION == lg.getDirectionType()) {
            shape = ShapeFactory.getInstace().createSemiRoundRect(width - insets.right, 1, insets.right - 1,
                    height - 2, this.radius, this.radius, ShapeFactory.RIGHT_ORIENTATION);
        } else {
            shape = ShapeFactory.getInstace().createRectangle(width - insets.right, insets.top, insets.right - 1,
                    height - insets.top - insets.bottom);
        }
