...
public class OntimizeStyle extends SynthStyle {
     ...
     */
    protected static final Object NULL = '\0';
    /**
     ...
     * getColorForState.</p>
     */
    protected static final Color DEFAULT_COLOR = new ColorUIResource(Color.BLACK);
    /**
     * Simple Comparator for ordering the RuntimeStates according to their
     * rank.
     */
    protected static final Comparator<RuntimeState> STATE_COMPARATOR =
     ...
     */
    protected CacheKey tmpKey = new CacheKey("", 0);
     ...
     */
    OntimizeStyle(String prefix, JComponent c) {
        this.component = c;
        this.prefix = prefix;
...
//        super.installDefaults(ctx);
        if (!ctx.getRegion().isSubregion()) {
            JComponent c = ctx.getComponent();
            Region region = ctx.getRegion();
            Font font = c.getFont();

            if (font == null || (font instanceof UIResource)) {
                c.setFont(getFontForState(ctx));
            }
            Color background = c.getBackground();
            if (background == null || (background instanceof UIResource)) {
                c.setBackground(getColorForState(ctx,
                                                 ColorType.BACKGROUND));
            }
            Color foreground = c.getForeground();
            if (foreground == null || (foreground instanceof UIResource)) {
                c.setForeground(getColorForState(ctx, 
                         ColorType.FOREGROUND));
            }
            LookAndFeel.installProperty(c, "opaque", Boolean.valueOf(isOpaque(ctx)));
        }
    }
    ...
    static void uninitialize() {
        ...
        PropertyChangeListener pcl = (PropertyChangeListener)
                ctx.get("OntimizeStyle.defaults.pcl");
        ...
        ctx.remove("OntimizeStyle.defaults.pcl");
        
        // clear out the compiled defaults
        ctx.put("OntimizeStyle.defaults", null);
    }
    ...
    protected void validate() {
        ...
        Map<String, TreeMap<String, Object>> compiledDefaults =
                (Map<String, TreeMap<String, Object>>)
                    ctx.get("OntimizeStyle.defaults");
        ...
        if (compiledDefaults == null) {
            ...
            if (pcl == null) {
                ...
                ctx.put("OntimizeStyle.defaults.pcl", pcl);
            }
            
            // store the defaults for reuse
            ctx.put("OntimizeStyle.defaults", compiledDefaults);
        }
        ...
        if (component != null) {
            Object o = component.getClientProperty("Ontimize.Overrides");
            if (o instanceof UIDefaults) {
                Object i = component.getClientProperty(
                        "Ontimize.Overrides.InheritDefaults");
     ...
     * @param d
     */
    protected void compileDefaults(
            Map<String, TreeMap<String,Object>> compiledDefaults,
            UIDefaults d) {
     ...
     */
    protected void init(Values v, TreeMap<String, Object> myDefaults) {
    ...
    protected Painter getPainter(TreeMap<String, Object> defaults, String key) {
     ...
     * </ul>
     */
    @Override public Color getColorForState(SynthContext ctx, ColorType type) {
        String key = null;
     ...
     */
    protected Values getValues(SynthContext ctx) {
        validate();
        return values;
    }
     ...
     */
    protected boolean contains(String[] names, String name) {
        assert name != null;
     ...
     * @return
     */
    protected int getExtendedState(SynthContext ctx, Values v) {
        JComponent c = ctx.getComponent();
        int xstate = 0;
        int mask = 1;
     ...
     * @return
     */
    protected RuntimeState getNextState(RuntimeState[] states,
                                      int[] lastState,
                                      int xstate) {
     ...
     * style.
     */
    protected class RuntimeState implements Cloneable {
        int state;
        ...
        protected RuntimeState(int state, String stateName) {
            this.state = state;
     ...
     */
    protected static class DefaultsListener implements PropertyChangeListener {
        @Override
        public void propertyChange(PropertyChangeEvent evt) {
            AppContext.getAppContext().put("OntimizeStyle.defaults", null);
        }
    }
}
